...
/* Algoritmo Radixsort com base fixa 256
 *  ou blocos de 8 bits
 *
 * Parametros:
 *      Record *v: vetor de structs com campo chave = key
 *      int N: tamanho do vetor
*/
void radixsort256(Record *v, int N){
    
    //Radixsort: executa multiplas vezes o counting sort (ou bucket sort)

    // 1 - Alocar vetores de contagem e auxiliares
    //      k = 256 valores possiveis
    
    int C[] = {0}; //contagem
    int pos[256];  //contagem acumulada
    Record *vcop = (Record*) malloc(sizeof(Record) * N); //copia

    //Particionar a chave em dígitos segundo uma base (neste caso, 256)
    // 32 bits: 4 blocos de 8 bits
    // wwwwwwwwzzzzzzzzyyyyyyyyxxxxxxxx
    //                         &&&&&&&& 
    // 00000000000000000000000011111111 == 255
    // 000000000000000000000000xxxxxxxx -> chave

    //Fazendo um shift de 8 bits pra direita, copia apenas os y
    // 00000000wwwwwwwwzzzzzzzzyyyyyyyyxxxxxxxx  >> 8
    //                         &&&&&&&& 
    // 00000000000000000000000011111111 == 255
    // 000000000000000000000000xxxxxxxx -> chave
    for (int shift = 0; shift <= 24; shift += 8){
        ...
        //2a - contagem + copia
        for (int j = 0; j < N; j++){
            //extrair um bloco de 8 bits
            short k = (v[j].key >> shift) & 255;

            C[k]++; //incrmenta - contando
            vcop[j] = v[j];
        }

        //2b - contagem acumulada
        
        pos[0] = 0; //inicialização
        for (int i=1; i < 256; i++){
            pos[i] = pos[i-1] + C[i-1];
            C[i-1] = 0; //zera contagem para a próxima passada
        }

        //2c - copio os elementos na posicao correta (pos)
        for (int j=0; j < N; j++){
            short k = (vcop[j].key >> shift) & 255;

            v[pos[k]] = vcop[j]; //ordena na posicao de k
